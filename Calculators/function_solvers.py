...
def total_atmospheric_escape_calculator(distance,r_xuv,eta,m_p,r_p,l_bol,dt,t_end,t_sat):
    t_end = t_end*365.25*24*3600 #Convert from years to seconds
    dt = dt*365.25*24*3600 #Convert from years to seconds
    t_sat = t_sat*365.25*24*3600 #Convert from years to seconds
...
def total_atmospheric_escape_integrator(distances,r_xuv,eta,m_p,r_p,l_bol,dt,t_end,t_sat):
    ...
    while t < t_end:
        ...
        f_xuv = calculate_stellar_fluxes(l_xuv, distances)
...
def calculate_stellar_fluxes(l_xuv, distances):
    """Calculates the time dependant stellar fluxes"""
    f_xuv = l_xuv / (4*pi*distances**2)
    return f_xuv

def calculate_l_xuv(l_bol,t=1,t_sat=1):
    """Calculates the xuv-luminosity from a star"""
    xuv_bol_sat = 1e-4 #Fraction of the bolometric luminosity that is emitted in xuv.

    if t > t_sat:
        xuv_bol = xuv_bol_sat*(t/t_sat)**(-1.23)
    else:
        xuv_bol = xuv_bol_sat
...
def calculate_total_mass_loss(eta, m_p, r_xuv, r_p, l_bol, l_q, t_sat, t_end, gamma, distances):
